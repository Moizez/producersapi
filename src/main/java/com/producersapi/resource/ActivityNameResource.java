...
public class ActivityNameResource extends Response<ActivityName> implements EntityResource<ActivityName> {
	...
	public ResponseEntity<ActivityName> findByName(@PathVariable("name") String name) {
		ActivityName activityName = service.findByLabel(name);
		if(activityName !=null) {
			return ResponseEntity.ok(activityName);
		} else {
			return ResponseEntity.notFound().build();
		}
